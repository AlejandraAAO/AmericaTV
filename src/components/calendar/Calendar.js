             ...
             this.state.data.map((data, i) => {
               return (
                 <tr key={`data${i}`}>
                   ...
                   {
                     arrayDay.map((day, j) => {
                       return (<td key={`day${j}`} className={`td text-secondary hour${i}`}><a href="#" id={i + "-" + day + "-" + data[day].name + "-" + data[day].price} onClick={this.handleClick.bind(this)}>{data[day].name}</a></td>)
                     })
                   }
                 </tr>
               )
